	...
	namespace detail
	{
		...
		class Range
		{
			...
			class Iter
			{
				...
				Iter(const ucl_object_t *arr, const ucl_iterate_type type) : array(arr), iterate_type(type)
				{
			...
			public:
			Range(const ucl_object_t *arr, const ucl_iterate_type type = UCL_ITERATE_BOTH) : array(arr), iterate_type(type)
			{
			}
			Iter begin()
			{
		...
		class DurationAdaptor
		{
			...
			operator std::chrono::seconds()
			{
				...
				return std::chrono::seconds(static_cast<long long>(ucl_object_todouble(obj)));
			}
		};

		template<typename NumberType, auto(*Conversion)(const ucl_object_t*) = ucl_object_toint>
		class NumberAdaptor
		{
			const ucl_object_t *obj;
			public:
		...
		class EnumValueMap
		{
			...
			{
				static_assert(std::is_same_v<Value, std::remove_reference_t<decltype(get<Element>(kvps).val)>>,
